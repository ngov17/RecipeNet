...
def Attention_Matrix(K, Q, use_mask=False):
    ...
    d_k = K.shape[2]
    atten_matrix = tf.matmul(Q, K_t)
    atten_matrix = tf.multiply(atten_matrix, 1 / math.sqrt(d_k))
    if use_mask:
        atten_matrix += atten_mask

    return tf.nn.softmax(atten_matrix)
...
class Atten_Head(tf.keras.layers.Layer):
    def __init__(self, input_size, output_size, use_mask):
        ...
        self.Wk = self.add_weight(shape=[input_size, output_size])
        self.Wv = self.add_weight(shape=[input_size, output_size])
        self.Wq = self.add_weight(shape=[input_size, output_size])
    ...
    def call(self, inputs_for_keys, inputs_for_values, inputs_for_queries):
        ...
        atten_matrix = Attention_Matrix(K, Q, use_mask=self.use_mask)
        self_atten = tf.matmul(atten_matrix, V)
